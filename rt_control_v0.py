...
rl_model_path     = base_path + '/weights/rl/rt_control/single_wide/best_model.zip' if wide else base_path + '/weights/rl/rt_control/single/best_model.zip'
max_models = 10
init_models = 4
max_shape_models = 4
...
# RL setting
if wide:
    low_state  = [0.3, 0.0, 0.0,0.0, 1.6,0.15, 0.5,1.265,2.14,0.8,4.0,0.80,0.8,4.0,0.80]
    high_state = [0.8,1.75,1.75,1.5,1.95, 0.5,0.85, 1.36, 2.3,2.1,7.0,1.05,2.1,7.0,1.05]
    low_action  = [0.3, 0.0, 0.0,0.0, 1.6,0.15,0.5 ,1.265,2.14]
    high_action = [0.8,1.75,1.75,1.5,1.95, 0.5,0.85, 1.36, 2.3]
else:
    low_state   = [0.35, 0.0, 0.0,0.0,1.65,0.15, 0.5,1.265,2.18,1.1,3.8,0.84,1.1,3.8,0.84]
    high_state  = [0.75,1.75,1.75,1.5,1.95, 0.5,0.85, 1.36,2.29,2.1,6.2,1.06,2.1,6.2,1.06]
...
class KSTARWidget(QDialog):
    ...
    def autoControl(self):
        ...
        new_action = self.rl_model.predict(observation)
        for i, idx in enumerate(idx_convert):
            self.inputSliderDict[input_params[idx]].setValue(f2i(new_action[i]))
